...
# chmod +x HBB_mapped_QC_reads.sh
# sed -i -e 's/\r$//' tHBB_mapped_QC_reads.sh
# ./HBB_mapped_QC_reads.sh /media/localarchive/m6A-P2S-Arraystar/10min-A
# for dir in /media/localarchive/m6A-P2S-Arraystar/*; do if [ -d "$dir" ]; then ./test_HBB_mapped.sh "$dir"& fi done
...
# Iterate over each set basecalled_dirs
	...
	if [[ -d "${basecalled_dirs}" ]]; then
		...
		echo -e "counting of all aligned..."
        ...
        echo -e "counting of successfully aligned..."
        ...
        mapped_gcount=$(eval "$count_mapped_reads_to_Gene")
       	echo -e "counting of HBB gene mapped..."
        count_mapped_HBB="samtools view -F 4 -c $basecalled_dirs/data_processing/output_sorted_$setname.bam \
		ENST00000335295.4 ENST00000647020.1 ENST00000485743.1 ENST00000380315.2 ENST00000633227.1 ENST00000475226.1" #successfully mapped HBB
        mapped_HBB=$(eval "$count_mapped_HBB")
		...
		# Calculate the ratio of mapped reads to all reads
        ratio_HBB_mapped=$(awk "BEGIN {printf \"%.2f\", $mapped_HBB/$all_gcount*100}")
		
		# Print the results
        echo "All reads: $all_gcount"
        echo "Mapped reads: $mapped_gcount"
		echo "Ratio of mapped HBB reads: $ratio_HBB_mapped%"
...
###################################################### Manual command ###########################################################
# samtools view -c -F 0x4 /media/localarchive/m6A-P2S-Arraystar/path_to_aligned_bam/index_aligned.bam ENST00000335295.4 ENST00000647020.1 ENST00000485743.1 ENST00000380315.2 ENST00000633227.1 ENST00000475226.1
