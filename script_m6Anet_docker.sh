#!/bin/bash
# chmod +x script_m6Anet_docker.sh
# sed -i -e 's/\r$//' script_m6Anet_docker.sh
# ./script_m6Anet_docker.sh /media/localarchive/m6A-P2S-Arraystar/10min-A
...
# Iterate over each set basecalled_dirs
	...
	if [[ -d "${basecalled_dirs}" ]]; then
		...
		sudo docker run --rm -v /media/localarchive:/localarchive quay.io/biocontainers/nanopolish:0.14.0--h773013f_3 nanopolish index \
        ...
        sudo docker run --rm -v /media/localarchive:/localarchive quay.io/biocontainers/nanopolish:0.14.0--h773013f_3 nanopolish eventalign \
