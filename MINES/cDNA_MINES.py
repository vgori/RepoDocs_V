...
df=df[df['start']>0]
df = df[["chr", "start", "stop", "drach_coordinate", "drach_kmer", "pos", "strand", 4]] 
...
model_list = pd.read_pickle(args.kmer_models)

#with open (args.kmer_models, encoding="utf-32-be", errors='ignore') as f: 
#    contents = f.read()
#k_models=args.kmer_models
#model_list = pd.read_csv(k_models, header=None, names=['file'], sep='\t')
# model_list = pd.read_csv(open(args.kmer_models, 'rb'), header=None, names=['file'], encoding='utf-32', engine='c', parse_dates=True )
...
for kmer in set(df_final['kmer']):
   ...
   # loaded_model = joblib.load(os.path.dirname(args.kmer_models)+'/'+fname)
    loaded_model = joblib.load(args.kmer_models)
