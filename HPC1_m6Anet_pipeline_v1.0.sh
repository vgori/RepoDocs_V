...
# chmod +x Dorado_script_m6Anet_docker_SdStr_v4.sh
# sed -i -e 's/\r$//' Dorado_script_m6Anet_docker_SdStr_v4.sh
# ./Dorado_script_m6Anet_docker_SdStr_v3.sh /buffer_drive/Adult-S003-0GY &\
# Change the reference path home/cmb-group/upni/media/cmb-group/venus/human_reference/transcriptome# ls Homo_sapiens.GRCh38.112.gtf  Homo_sapiens.GRCh38.cdna.all.fa
# not medida but buffer drive folder
...
# Iterate over each set basecalled_dirs
	...
	if [[ -d "${basecalled_dirs}" ]]; then
        ...
        sudo docker run --rm -v /buffer_drive:/buffer_drive quay.io/biocontainers/nanopolish:0.14.0--h773013f_3 nanopolish index \
        ...
        sudo docker run --rm -v /buffer_drive:/buffer_drive quay.io/biocontainers/nanopolish:0.14.0--h773013f_3 nanopolish eventalign \
        ...
        sudo docker run --rm -v /buffer_drive:/buffer_drive quay.io/biocontainers/m6anet:2.1.0--pyhdfd78af_0 m6anet dataprep --eventalign $basecalled_dirs/processing_nanopolish/eventalign.txt \
		...
		sudo docker run --rm -v /buffer_drive:/buffer_drive --shm-size=900g quay.io/biocontainers/m6anet:2.1.0--pyhdfd78af_0 m6anet inference --input_dir $basecalled_dirs/processing_m6anet/output_dataprep/ \
