...
This software is designed for transcriptome analysis, including assembly and annotation of cDNA (and direct RNA in the future) sequencing data, as well as differential expression analysis.
...
All outputs are collected in the 'data_processing' folder. The output from Salmon is in quant.sf format, which is converted to quant.tsv. Additionally, there is an output from samtools named stat_aligned_<sample_name>.txt containing the mapped reads (third column). These outputs can be used with DEXSeq or edgeR R packages for downstream analysis.
