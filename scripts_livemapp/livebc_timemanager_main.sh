...
while true; do
    ...
    # Wait until files appear in the mapped_dirfolder
    if ls "$SHARED_FOLDER/mapped_dir/singlefastq_bam_bai"/*.bam >/dev/null 2>&1; then
    for bam_file in "$SHARED_FOLDER/mapped_dir/singlefastq_bam_bai"/*.bam
        do
            ...
            tput rc
			mv "$SHARED_FOLDER/mapped_dir/singlefastq_bam_bai"/*.bam "$SHARED_FOLDER/mapped_dir/bam_saved/"
